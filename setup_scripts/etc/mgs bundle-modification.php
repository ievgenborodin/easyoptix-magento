...
_Draft_;
		...
		while (($data_csv = fgetcsv($handle,1000000,",")) !== FALSE) {
			if($row_count != 0){
				...
				$bundleProduct = Mage::getModel('catalog/product')->loadByAttribute('sku', 'rx-' . $data_csv[1]);
				...
				if(!$bundleProduct){
					...
					try{
						...
						$media_gallery = $frame->getMediaGalleryImages();
						...
						$p = array(
				            ...
				            'url_key' => 'rx-' .  $frame->getData('url_key'),
				            'type_id' => 'bundle',
							...
							'frame_type' => $frame->getData('frame_type'),
							...
							'lens_type' => $frame->getData('lens_type'),
							'color' => $frame->getData('color'),
						...
						foreach ($media_gallery as $image) {
					    	if( $path = $image->getPath() ) {
					    		if($m==0)
					    		{
					    			$bundleProduct->addImageToMediaGallery($path, array('image','thumbnail','small_image'), false, false);
					    		}
					    		else 
					    		{
					    			$bundleProduct->addImageToMediaGallery($path, NULL, false, false);
					    		}
					        	$m++;
					       	}
					   	}						
